        ...
        {data.networkPointsOk === false && (
            <div className="space-y-4 mt-6 border-t pt-4">
                <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={addNetworkPoint} className="text-xs bg-red-100 text-red-700 px-3 py-1 rounded-full hover:bg-red-200 flex items-center gap-1 font-medium"><Plus size={14} /> Adicionar Ponto</button>
                </div>
                {data.problematicNetworkPoints.map((np) => (
                    <div key={np.id} className="bg-red-50 p-4 rounded-lg border border-red-200 relative">
                        <button type="button" onClick={() => removeNetworkPoint(np.id)} className="absolute top-2 right-2 text-red-400 hover:text-red-600 p-1"><Trash2 size={16} /></button>
