...
export const Autocomplete: React.FC<AutocompleteProps> = ({ label, value, onChange, placeholder }) => {
  ...
  useEffect(() => {
    ...
    const currentUserDataRaw = localStorage.getItem("infracheck_user_data");
    const currentUserData = currentUserDataRaw ? JSON.parse(currentUserDataRaw) : null;
    const currentRegion = currentUserData ? currentUserData.region : "";

    const locsRaw = localStorage.getItem('infracheck_locations');
    const predefinedLocs = locsRaw ? JSON.parse(locsRaw) : [];

    const predefinedNames = currentRegion
        ? predefinedLocs.filter((l: any) => l.region === currentRegion).map((l: any) => l.name)
        : predefinedLocs.map((l: any) => l.name);

    const allLocations = Array.from(new Set([...predefinedNames, ...customLocations]));
  ...
  const handleBlur = () => {
      ...
      if (value && value.trim().length > 1) {
          ...
          const locsRaw = localStorage.getItem('infracheck_locations');
          const predefinedLocs = locsRaw ? JSON.parse(locsRaw) : [];
          const predefinedNames = predefinedLocs.map((l: any) => l.name);
          const allLocations = new Set([...predefinedNames, ...customLocations]);
